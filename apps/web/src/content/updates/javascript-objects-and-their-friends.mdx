...
Let's say you have a `User` entity that usually contains `name`, `email`, `age`, `isVerified`, etc. Those fields typically span at least three data types (`string`, `number`, and `boolean`). Sure, you _could_ store each value in its own variable but it's much cleaner to group them into a single object:
...
Now the `user` variable holds everything in one place. You can access values with `user.name` and you'll get `John Doe`. If you want to remove a field, you can do `delete user.name` and the name key will be removed from the object.
...
## Let's start with `Object.prototype`

When people say "JavaScript objects are simple", they usually mean the syntax is simple. `{}` feels like a plain bag of data.
...
That prototype link is why objects can inherit behavior, why some properties "exists" even when you didn't define them, and why you sometimes see "spooky" stuff during debugging such as `toString`, `hasOwnProperty`, etc.
...
That object's prototype is `Object.prototype`, it comes by default from JavaScript. You can figuring out the prototype by simply: `console.log(Object.getPrototypeOf(o) === Object.prototype);`. And that's why this works:

```js
({}).toString();
```

Because you didn't define `toString`. `Object.prototype` did, and your object inherits it through the prototype chain. So a "plain object" is never truly empty. It always comes with a prototype... unless you deliberately remove it.
...
Let's say you are using the user object above and trying to access the `name` property like this `user.name`. JavaScript basically does a lookup operation like this:
...
<figure>
	<Image
		src="/images/updates/javascript-object-and-its-friends/object-result.avif"
		width={1024}
		height={576}
...
We'll focus on `console.log(user)`, which can look a bit strange the first time you see it. That log shows the user object plus its prototype. And you can see we even have a nested prototype, because we declared `__proto__` inside our `user` object.
...
Now let's remove the `__proto__` object inside our `user`:
...
In the code above you'll see the user object only contains one prototype, which is the default JS prototype that contains multiple property we can use such as `toString` which used for _stringify_ the object (if you call that function you'll see something like `"[object Object]"`) its a JS default object prototype.
...
So when you trying to call `user.toString()` it will return `Hello string!` instead `"[object Object]"` from our object. Simply lovely!
...
1.  Prototype != copying
    ...
    You can see the `a.greet()` and `b.greet()` return the same value which is `hello`, that is because the a and b object share the same prototype.
...
2.  It exists vs it's mine
    ...
    If payload has a weird prototype (or your intern has their own mind to write JS object), "role" in payload might be true even when it shouldn't be. That's why my default answer for "does this object actually contain this field?" is `Object.hasOwn(object, key)` which is more direct and it matches what you usually mean.

3.  `for...in` walks inherited enumerable properties
    ...
    So, if you only want to get the own keys, prefer `Object.keys()` or `Object.entries()`.

4.  You can create object without prototype (if you want to)

    Yes, we can create object without prototype by simply using `Object.create(null)` and assign the prototype easily.

---

## `Map` for more flexible data
...
`[0]` and `['0']` return the same value because JavaScript converts keys to strings. Only `symbol` doesn't get converted.
...
That result is expected because JavaScript stringifies object keys, and _stringifying_ an object returns `[object Object]`. Even if you add a different object key `obj[{ age: 19 }] = 'Second'`, JavaScript still treats it as the same property key after stringification.

That behavior makes objects a poor fit for a "true hash map", where keys can be _anything_, not just strings. And that's where `Map()` comes in.
...
The main differences is Map doesn't come with any other things by default, while objects often come with a prototype (if we declare the object by `{}` or `Object.create(obj)`), also it could have a key by any primitive and object type instead of string or symbol only.
...
Those code will return the same Map that contains `1` and `hello` key. You can see that map contains two key with different types which is `number` and `string`.

### Retrieving, deleting, and checking values

Once you have a `Map`, the API is pretty straightforward:

- `map.get(key)` to read
- `map.set(key, value)` to write
- `map.delete(key)` to remove
- `map.has(key)` to check existence
- `map.clear()` to wipe everything
...
1. Explicitly convert them into something iterable (like `Object.entries(obj)`), or
2. Implement `[Symbol.iterator]` yourself (we'll come back to this in the next section when we talk about `Symbols` especially `Symbol.iterator`), which is what JavaScript checks to decide whether something is iterable

### When to choose `Map` over `Object`?

Honestly it depends to your needs. But, for myself I'll use `Map` if:
...
And I'll choose `Object` over `Map` if:

1. The data is more like a "record" with a stable shape such as `user.name`, `config.timeout`, or
...
A `Symbol` is a primitive type that produces a unique value every time you create it even if you give it the same description.
...
Symbols shine when you want to attach extra information to an object without turning that information into part of the "surface" area. In real code, this happens more often than you'd think maybe when you build a tiny library, you return an object with a nice API, and you need to stash some internal state somewhere: a cache, a timestamp, a plugin context, a counter, a reference to the original config. If you use normal string keys for that (`_internal`, `_meta`, `__state`), it works... until it doesn't. Users can accidentally overwrite it, your internal keys show up in logs and `Object.keys()`, and at some point you're stuck defending your "internal" naming convention forever (okay that too much, but could be happen, right?).

A `Symbol` key "overcome" that whole mess. It gives you a property name that won't collide with normal keys and could be "invisible" in some ways. So your object can carry internal baggage without looking heavier from the outside (if that the right analogy (idk)).

### `Symbol.for` and `Symbol.keyFor` when you _want_ a shared Symbol

By default, `Symbol()` always creates a fresh Symbol. That's why the "invisible data" pattern works so nobody can override your internal slot unless they already have the exact Symbol reference.

Sometimes though, you want different modules (or different copies of a library) to agree on the _same_ `Symbol` key without passing it around manually. And that's what `Symbol.for` does. It looks up a Symbol in the **global Symbol registry** using a string key, and returns the same Symbol every time.
...
`Symbol.keyFor` goes the other way, when we given a Symbol from the registry, it'll returns the registry key.
...
But, `Symbol.keyFor` only works for Symbols created via `Symbol.for`. For normal Symbols, it returns undefined:

```js
console.log(Symbol.keyFor(Symbol("x"))); // undefined
```

In practice, I will use normal `Symbol()` for a private-ish internals and `Symbol.for()` for a shared contracts across boundaries. Use the registry deliberately, because once the key becomes "public", other code can access that too.
...
You can see the `newStore.getValue()` implementation can be overrided by the user simply by passing new `getValue()` option but, the internal state couldn't changed even you know the symbol description.

Your internal state lives behind `[INTERNAL]` and `[STATE]` keys, which are Symbols. Even if someone "guesses" the description `"internal"`, they're still creating a _different_ Symbol, so they won't hit your internal slot. In practice, this keeps your internal storage from being accidentally replaced by userland options, while still letting users override the parts you meant to be overrideable.

Symbol-backed fields also doesn't show up in `Object.keys()`. So if someone prints your store object, they'll mostly see the public API (`getValue`, `override`, etc.), not your internal data. When you do want to inspect everything during debugging, `Reflect.ownKeys(store)` will show both string keys and symbol keys.
...
JavaScript itself uses Symbols as _hooks_ to enable language features. These are called **well known symbols**. It is a "system" symbols used by JavaScript internally, and we can use them, or maybe re-implementing it by ourselves, such as:
...
But, in this article I want to focus on `Symbol.iterator`.
...
That's because arrays is _iterable_ and in JavaScript, a value is called iterable when it implements the _iterator protocol_ and it checks for one specific hook which is `value[Symbol.iterator]`. That iterator must expose a `next()` method and each call to `next()` returns an object with two fields: `value` and `done`. The `value` is the next item in the sequence, and `done` tells JavaScript whether the sequence has finished. As long as `done` is `false`, `for...of` keeps going and uses `value` as the loop variable. Once `done` becomes `true`, the loop stops.

Arrays work with `for...of` because they already implement this protocol. Plain objects don't, so JavaScript has no built in to iterate over `{}` unless you convert it first (like `Object.entries(obj)`) or implement `Symbol.iterator` yourself.
...
3. Keep calling `iterator.next()` to pull values one by one
4. And stop when `next()` returns `{ done: true }`

### Let's try making a plain object iterable

Like I said earlier, a plain object doesn't come with `Symbol.iterator`. That's why `for...of` throws on `{}` by default. But, lets try to experiment by implementing `Symbol.iterator` in object by ourselves.

At the simplest level, writing an iterable means returning an **iterator** which is an object with `next()` method that produces `{ value, done }`. The iterator keeps track of its current position, returns the next value, and eventually says if they already completed.
...
A generator (`function*`) is the easiest way to implement this because it builds the iterator for you. Each yield becomes one step in the iteration.
...
- If the data feels like a **record** with a stable shape that you'll read/write with dot access (`user.name`, `config.timeout`), pass around, maybe serialize to JSON then a plain `Object` is still the most fit for me.

- If the data feels like a **collection**; keys are dynamic, you add/remove entries a lot, you care about iteration, or your keys aren't naturally strings, then `Map` usually makes the code cleaner. `Map` doesn't secretly _stringify_ keys, it gives you `size`, and iterating it is a easy thing instead of "convert to entries first and then loop".

- If you need to attach **internal metadata** to an object (state, plugin data, diagnostics, cache timestamps) without turning it into part of the public surface, then `Symbol` keys are a really nice move. You avoid key collisions, you avoid leaking internal fields into `Object.keys()`, doesn't want the "old" naming-convention (`_internal`, `__meta`, etc.). And when the key needs to be shared across modules or bundles, `Symbol.for` gives you a deliberate "global contract" key.

- And for iteration specifically, if you're building a custom structure that genuinely wants to behave like a collection, implementing `Symbol.iterator` makes it plug into `for...of`, spread, and other iterable consumers. For plain record objects, I usually just loop over `Object.entries(obj)` which is more known by other people (and simple).
