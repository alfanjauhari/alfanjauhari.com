...
### `Symbol.for` and `Symbol.keyFor` when you *want* a shared Symbol
...
Sometimes though, you want different modules (or different copies of a library) to agree on the *same* `Symbol` key without passing it around manually. And that's what `Symbol.for` does. It looks up a Symbol in the **global Symbol registry** using a string key, and returns the same Symbol every time.
